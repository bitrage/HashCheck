...
import functools
import platform
import hash_check_qrc  # @UnusedImport
...
class Checksum_Calculator(QtWidgets.QMainWindow):
    ...
    def action_export_index(self):
        filepath, file_filter = QtWidgets.QFileDialog.getSaveFileName(self, "Export checksums to index file", self.last_path, "SFV file (*.sfv);;MD5 file (*.md5);;SHA1 file (*.sha1)")
        if not filepath:
            return False
        filter_to_algorithm = {"SFV file (*.sfv)":"CRC32", "MD5 file (*.md5)":"MD5", "SHA1 file (*.sha1)":"SHA1"}
        set_hash_algorithm = filter_to_algorithm[file_filter]
        self.last_path = os.path.dirname(filepath)
        hashlist = []
    ...
    def checksum_calc_thread(self):
        while True:
            ...
            hash_algorithm = self.hashsum(self.algo_dict[algorithm], url)
            self.signal_treeview_add_new_file.emit(algorithm, url, hash_algorithm, reference)
            self.action_enabler()
    ...
    def parse_hash_list_file(self, filepath, set_hash_algorithm):
        ...
        with open(filepath) as f:
            for line in f:
                hash_algorithm, filename = ("", "")
                if line.startswith(";"):
                    continue
                split_line = line.split()
                if len(split_line) == 2:
                    if hash_pattern.match(split_line[0]):
                        hash_algorithm, filename = split_line
                    elif hash_pattern.match(split_line[1]):
                        filename, hash_algorithm = split_line
                if "*" in filename:
                    filename = filename.split("*")[-1]
                hash_reference = hash_algorithm.upper()
    ...
    def extract_hash_from_filename(self, filepath):
        ...
        if hash_pattern.match(os.path.basename(filepath)):
            hash_algorithm = hash_pattern.match(os.path.basename(filepath)).group(1)
            if len(hash_algorithm) in algo_length_dict:
                hash_algo = algo_length_dict[len(hash_algorithm)]
        return (hash_algorithm, hash_algo)
            
    def treeview_add_new_file(self, algorithm, filepath, hash_algorithm, hash_reference=None):
        row = QtWidgets.QTreeWidgetItem()
        row.filepath = filepath
        row.setIcon(0, self.icons["blank"])
        row.setText(0, os.path.basename(filepath))
        row.setText(1, algorithm)
        row.setText(2, hash_algorithm)
        if hash_algorithm == "N/A":
            row.setIcon(0, self.icons["error"])
        elif self.abort_check:
            self.abort_check = False
            row.setIcon(0, self.icons["question"])
        elif hash_reference:
            if hash_reference == hash_algorithm:
    ...
    def hashsum(self, hashname, filename):
        self.abort_check = False
        try:
            hash_algorithm = getattr(hashlib, hashname)()
        except AttributeError:
            hash_algorithm = hashlib.new(hashname)
        ...
        total_read = 0
        last_output = 0
        chunk_size = 128*hash_algorithm.block_size
